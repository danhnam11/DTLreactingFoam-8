...
) const
{
    ...
    // concentration print out - JH
/*
    Info << "DEBUG: c values:" << endl;
    for (label i = 0; i < c.size(); ++i)
    {
        Info << "DEBUG: c[" << i << "] = " << c[i] << endl;
    }
*/
    // Pressure Temperature print out - JH
    //Info << "DEBUG: Pressure p = " << p << ", Temperature T = " << T << endl;
    ...
    // omega function results print out - JH
    //Info << "DEBUG: dcdt values after omega calculation:" << endl;
/*
    for (label i = 0; i < dcdt.size(); ++i)
    {
        Info << "DEBUG: dcdt[" << i << "] = " << dcdt[i] << endl;
    }
*/
    // Constant pressure
    // dT/dt = ...
    ...
    // DEBUG: 최종 dcdt 값 출력 - JH
/*
    Info << "DEBUG: Final dcdt values:" << endl;
    for (label i = 0; i < dcdt.size(); ++i)
    {
        Info << "DEBUG: dcdt[" << i << "] = " << dcdt[i] << endl;
    }
...
template<class ReactionThermo, class ThermoType>
Foam::tmp<Foam::DimensionedField<Foam::scalar, Foam::volMesh>>
Foam::StandardChemistryModel<ReactionThermo, ThermoType>::reactionRate
(
    const label ri
) const
{
